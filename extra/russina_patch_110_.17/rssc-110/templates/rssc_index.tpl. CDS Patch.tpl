<{* $Id: rssc_index.html.\040CDS\040Patch.html,v 1.1 2012/04/08 23:42:20 ohwada Exp $ *}>
...
<div align="center">
    <form action="<{$xoops_url}>/modules/<{$dirname}>/index.php" method="get">
        ...
        <select id="andor" name="andor" size="1">
            <option value="AND"
            <{$rssc_and}> ><{$lang_all}></option>
            <option value="OR"
            <{$rssc_or}> ><{$lang_any}></option>
            <option value="exact"
            <{$rssc_exact}> ><{$lang_exact}></option>
        </select>
...
<hr/>
<div class="rssc_execution_time"><{$smarty.const._RSSC_EXECUTION_TIME}> : <{$execution_time}> <{$smarty.const._RSSC_SEC}></div>
<{if $is_module_admin && ($memory_usage > 0)}>
    <div class="rssc_memory_usage"><{$smarty.const._RSSC_MEMORY_USAGE}> : <{$memory_usage}> <{$smarty.const._RSSC_MB}></div>
    <{/if}>
