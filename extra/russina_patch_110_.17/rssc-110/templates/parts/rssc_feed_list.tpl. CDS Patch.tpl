<{* $Id: rssc_feed_list.html.\040CDS\040Patch.html,v 1.1 2012/04/08 23:42:20 ohwada Exp $ *}>

<{* --- feed loop begin --- *}>
<{foreach item=feed from=$feeds}>

    <div class="rssc_feed_list">
        <a href="<{$xoops_url}>/modules/<{$dirname}>/single_feed.php?fid=<{$feed.fid}>">
            <img src="<{$xoops_url}>/modules/<{$dirname}>/images/text.gif" border="0" alt="feed"/></a>

        <{if $is_module_admin && (($feed.ltype == 1)||($feed.lid == 0)) }>
        ...
        <{if $show_icon }>
        <a href="<{$xoops_url}>/modules/<{$dirname}>/single_link.php?lid=<{$feed.lid}>&amp;keywords=<{$keywords}>">
            <{if $feed.icon != ''}>
        ...
        <{if $show_site && ($feed.site_link != '') && ($feed.site_title != '') }>
        <{$smarty.const._RSSC_IN}>
        ...
        <a href="<{$feed.site_link}>" target="_blank"><{$feed.site_title}></a>
        <{/if}>
